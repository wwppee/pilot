...
const VALID_CAPABILITY = {
  ...
  sources: [{ type: "npm", ref: "npm:pi-subagents", mode: "L2-wrapped" }],
...
describe("capabilityDir", () => {
  it("returns path for valid kebab-case id", () => {
    ...
    expect(capabilityDir("subagent-orchestrator")).toMatch(
...
describe("CapabilitySchema", () => {
  ...
  it("rejects id with spaces", () => {
    expect(() =>
      CapabilitySchema.parse({ ...VALID_CAPABILITY, id: "plan mode" }),
    ).toThrow();
  });
...
describe("loadCapability + listCapabilities", () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(join(tempHome, ".pilot/capabilities/plan-mode"), {
      recursive: true,
    });
    mkdirSync(join(tempHome, ".pilot/capabilities/broken"), {
      recursive: true,
    });
