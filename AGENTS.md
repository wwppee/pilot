...
### 10.20 AGENTS.md self-rule 失守（v0.9.14 教训）

**症状**：AGENTS.md §1.1 / §6.1 写 "root 541 / web 214"，但实际 v0.9.13 已涨到 "root 655 / web 312"。落后 174 个 root test、98 个 web test。AGENTS.md 自己规定 "每次新 release → 回写新教训 + 校对测试数"，但这条规则连续 20+ 个 release 没执行。

**实际**：user 拿 GLM 5.2 跑 audit 一眼看穿，文档债立刻升到 P0（user 不信任"自己定的规则自己没执行"的项目）。

**防**：

- release commit 必带 AGENTS.md 校对（不只是 changelog + roadmap）
- tsc / tests / format / lint 之外加 "AGENTS.md 数字校对" 作为第 5 个 verify gate
- 如果 release 没动 AGENTS.md，commit message 必说明 "no AGENTS.md changes (no lesson learned this release)"

### 10.21 error state 走 `text-muted`（v0.9.14 教训）

**症状**：usePiSession 在 token 抓取失败时 `setError("token fetch failed: 503")` + `setState("error")`。/try 的 statusLabel 把 `session.error` 当 i18n key 传给 `<T>`，渲染出 "token fetch failed: 503" 原文。user 点连接按钮后**完全看不到反馈**——text 颜色是 `text-muted`（暗主题下几乎不可见）+ 字号 text-sm + 跟 idle 状态同位置。

**实际**：user 报"点击连接没反应" 实际是 click **有响应**但视觉上看不出变化。错误存在但被盲盒化。

**防**：

- 错误状态**永远**走 `var(--error)` 颜色 + 边框 + icon（不是 `text-muted`）
- 错误消息**直接渲染**（不要走 i18n key 路径当 fallback）
- 错误状态必带 **retry 按钮**，让 user 有下一步动作
- "状态会变但 UI 看不出" 比 "状态不变" 更阴险——后者 user 立刻发现，前者 user 当"我哪里做错了"自我怀疑

### 10.22 暗主题 inactive 按钮可见性（v0.9.14 教训）

**症状**：LanguageSwitcher 的 inactive 按钮用 `background: transparent` + `color: var(--text-muted)` + `fontWeight: 400`。暗主题下 transparent = 不可见，text-muted = 灰字淡到融背景。

**实际**：user 只看见 "EN"，看不见 "中"（其实有，只是淡到看不见）。报"en/zh按钮有点问题"。

**防**：

- toggle UI 的 inactive 项必须**保留视觉权重**——最低给个 `surface-2` 背景或 `text` 色（不是 `text-muted`）
