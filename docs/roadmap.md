...
| 修了什么                                                                                                                        | 根因                                                                                                                                                                | 修法                                                                   |
| ------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `/workflows/[id]` 500                                                                                                           | `app/workflows/[id]/layout.ts` 是 `autoLayout` 纯函数，但 Next.js 把 app/ 下任何 `layout.{ts,tsx}` 当 route layout 加载 → "default export is not a React Component" | git mv 到 `workflow-layout.ts` (同目录改名字，Next.js 不再匹配)        |
| 8 路由 200-该-404 (`/packages` `/sessions` `/capabilities` `/profiles` `/policy/*/edit` `/forge` `/avatars` `/wrappers/*/edit`) | 找不到资源时渲染 inline "not found" surface，HTTP 仍 200                                                                                                            | 每个 page 调 `notFound()` (next/navigation) → `app/not-found.tsx` 路由 |
| `/workflows/[id]` 加 server-side 存在性 check                                                                                   | 跟 client useEffect 重复 + race                                                                                                                                     | page.tsx 调 `api.workflow(id)`，null 就 notFound()                     |
| 9 unit test 锁 notFound 行为                                                                                                    | 未来 refactor 谁把 "inline 'not found' surface" 写回来，CI fail                                                                                                     | `not-found-routes.test.ts` (9 tests)                                   |

**⚠️ Next.js 16.2.x upstream bug 标注**（**不是 pilot bug**）：

- v0.9.15 body 渲染对（`error.notFound.title` 等全在），但 **HTTP 状态返 200 不是 404**
- 根因：`HTTPAccessFallbackBoundary` 在 segment tree 有 `loading.tsx` 时吞 notFound() status
- [issue #93008](https://github.com/vercel/next.js/issues/93008)
- 4 个 loading.tsx (`/loading.tsx` + `sessions` + `profiles` + `plans`) 覆盖所有动态路由
- workaround：删 loading.tsx（loss of UX）/ middleware 预检（重复 round trip）/ 等 Next.js
- v0.9.15 没在 pilot 端 hack——hack 端代价是 4 个 loading UX，且是 Next.js 责任

**用户 dogfooding 经验**：

- `pilot dashboard` 撞 bug #1 时 dev server 进坏状态，所有 client-side React 都被影响
- "v0.9.14 修了但按钮还不响应" 是 cache + dev server 状态问题，**不是 v0.9.14 fix 失效**
- 解决：`Ctrl-C` `pilot dashboard` + `mavis-trash .next` + 重启 + 浏览器 `Cmd+Shift+R`

**Stats**: root 655/655, web 323 → 331 (+8), tsc/lint/build clean, 1 new test file

**Defer v0.9.16+** (GLM 5.2 audit 提的 4 项还没动)：
