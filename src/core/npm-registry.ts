...
export async function searchPacks(opts: SearchOptions): Promise<Pack[]> {
  ...
  return json.objects.map((o) => ({
    ...
    ...(o.package.keywords !== undefined ? { keywords: o.package.keywords } : {}),
    ...(o.package.links?.npm !== undefined ? { repository: o.package.links.npm } : {}),
  }));
}
...
export async function getPack(name: string): Promise<Pack | null> {
  ...
  return {
    ...
    ...(json.time?.['1.0.0'] !== undefined
      ? { lastPublished: json.time[json['dist-tags'].latest] ?? json.time['1.0.0'] }
      : {}),
